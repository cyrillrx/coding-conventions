...
- E2E: critical journeys are covered by Maestro flows in `.maestro/flows/`, one kebab-case file per journey — `launchApp: clearState: true`, commented steps, closing assertion. Nodes addressed by identity expose a stable id instead of relying on localized text; on Android a Compose `testTag` is only visible to Maestro under `Modifier.semantics { testTagsAsResourceId = true }`, which applies to a subtree and must therefore be re-enabled inside every dialog (an `AlertDialog` is a separate window). They complement, never replace, unit tests, and are not a PR gate — install the app, then `maestro test .maestro/flows/` locally (Android/iOS only, no Desktop coverage).
